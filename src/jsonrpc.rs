...
/// A successful JSON-RPC response wrapping a `CallToolResult` with
/// `isError: true` — used so MCP clients can distinguish tool-level failures
/// from protocol errors and attempt recovery.
